...
import { SearchRoot, SearchInput, SearchResults, Suggestions, ChatRoot } from '@orama/ui/components'
            ...
            {searchTerm =>
              searchTerm ? (
                <div className="h-64 flex flex-col items-center justify-center">
                  ...
                  <p className="text-muted-foreground py-2">Try searching for something else!</p>
                </div>
              ) : (
                    ...
                    {promptSuggestions.map((suggestion, index) => (
                      <li
                        ...
                        className={`text-xs text-muted-foreground opacity-0 ${
                          mounted ? `suggestion-animate suggestion-delay-${index}` : ''
                        }`}
                      >
                        <Suggestions.Item
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-secondary-foreground min-w-0 mt-1">
                          {result.document?.content ? `${(result.document?.content as string).slice(0, 140)}...` : ''}
                        </p>
                      </div>
                    </SearchResults.Item>
                  ) : null}
