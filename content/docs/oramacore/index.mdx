...
<Cards>
    ...
    <Card icon={<WholeWordIcon className="dark:text-green-300 text-green-500" />} title='LLMs'>
        Built-in support for both local and remote large language models that can handle all sorts of operations on your data, from translating natural language queries into optimized OramaCore queries to running custom agents.
    </Card>
...
All in a single Docker Compose file.

### Configuration

To get started with OramaCore, you can use the default configuration. But if you want to customize it, you can do so by editing the `config.yml` file.
You can customize the system to fit your specific needs. Check out the [configuration](/docs/oramacore/guide/configuration) guide to learn more.
