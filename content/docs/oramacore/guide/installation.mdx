...
One thing to remember when using Docker is network management. When configuring [the OramaCore configuration file](/docs/oramacore/guide/configuration) (`config.yaml`), ensure that the `ai_server.host` is set to `python-ai-server`, and the `ai_server.llm.host` is set to `vllm`.
...
When you run the server, OramaCore will automatically download the required models specified in the [configuration file](/docs/oramacore/guide/configuration).
