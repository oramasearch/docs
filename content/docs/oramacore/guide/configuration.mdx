...
writer_side:
    ...
    config:
        ...
        # if not specified in the project creation
...
- `master_api_key`: The master API key used to authenticate the requests to the writer side. By default, it's set to an empty string. See more about the available API keys in the [API Keys](/docs/oramacore/guide/api-key) section.
- `config`: The configuration options for the writer side. Here are the available options:
  ...
  - `default_embedding_model`: The default embedding model used to calculate the embeddings if not specified in the project creation. By default, it's set to `BGESmall`. See more about the available models in the [Embedding Models](/docs/oramacore/customizations/text-embeddings) section.
...
- `default_model_group`: The default model group used to calculate the embeddings if not specified in the project creation. By default, it's set to `small`. See more about the available models in the [Embedding Models](/docs/oramacore/customizations/text-embeddings) section.
...
- `model`: the model used to run [Answer](/docs/oramacore/apis/answer-session) and [Reasoning](/docs/oramacore/apis/advanced-reasoning) Sessions. By default, it's set on `Qwen/Qwen2.5-3B-Instruct`.
