...
However, one way to improve performances and quality of your output is to use a more powerful language model. By default, Orama Cloud uses a local `gpt-oss 120b` model to perform all the NLP tasks, but you can select a different model by providing a `LlmConfig` object with the desired provider and model name:
