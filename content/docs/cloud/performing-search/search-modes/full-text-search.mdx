...
| `boost`      | The boost factors for different fields. Should be an object mapping field names to their boost values.                                                               | -              |
...
Orama uses **BM25F** for field boosting, which allows you to give different importance to different fields in your search queries.

For instance, if you're indexing a documentation website, each document may look like:

```json
{
  "title": "Learning field boosting on Orama",
  "content": "In this guide, we'll explore how to implement field boosting using Orama...",
  "tags": ["field boosting", "search", "Orama"],
  "path": "/docs/cloud/performing-search/search-modes/full-text-search#field-boosting"
}
```

You may want to give more weight to the `title` field compared to the `content` field, as users are more likely to find relevant information from the title.

You can do that by setting the `boost` property in your search query:
...
This way, Orama will prioritize results that match the `title` field more closely than those that match the `content` field.
