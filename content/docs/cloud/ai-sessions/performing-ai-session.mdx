...
Once you have imported your data into Orama Cloud, you can start performing AI sessions. AI sessions are a way to interact with your data using interfaces that are different from the traditional search operations—for example, chats, recommendation systems, and more.
...
By default, Orama will use the internal `gpt-oss 120b` LLM for performing AI sessions, but you can specify a different, external model by providing a `LLMConfig` object when creating the AI session:
